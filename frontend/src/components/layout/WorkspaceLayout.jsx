...
const WorkspaceLayoutContent = () => {
  ...
  const { activeWorkspace, boards, isWorkspaceLoading, showCreateBoard, setShowCreateBoard, createBoard } = useWorkspace();
  ...
  if (isWorkspaceLoading || !activeWorkspace) {
    return (
      <div className="flex h-dvh font-sans overflow-hidden bg-surface dark:bg-background">
        ...
        <div className="flex-1 flex flex-col h-full min-w-0 overflow-hidden animate-pulse">
          <div className="h-14 border-b border-outline-variant bg-surface-container-lowest animate-pulse px-4 flex items-center justify-between" />
          <div className="p-8 space-y-6 w-full max-w-[1280px] mx-auto">
